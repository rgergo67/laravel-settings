...
@section('content')
    <h1>@lang('settings::settings.title')
        <a href="{{ route('settings-admin.create') }}" class="btn btn-success btn-sm align-self-center ml-1">
        ...
        @foreach($settings as $setting)
            <tr>
                ...
                <td class="text-nowrap">
                    <a class="btn btn-sm btn-warning" href="{{ route('settings-admin.edit', $setting) }}">@lang('settings::settings.edit')</a>

                    {{ Form::model($setting, ['method' => 'DELETE', 'route' => ['settings-admin.destroy', $setting], 'class' => 'd-inline delete', 'data-confirm' => __('settings.settings::delete_confirmation')]) }}
